...
def detect_market_pattern(df, slope, W=15):
    ...
    higher_lows = (
        window['Low'].iloc[-5:].min() >
        window['Low'].iloc[:5].min()
    )

    lower_highs = (
        window['High'].iloc[-5:].max() <
        window['High'].iloc[:5].max()
    )
...
if st.button("## 🏆 Watchlist 共振排行榜"):
    ...
    for ticker, name in st.session_state.watchlist_dict.items():
        ...
        resonance_rows.append({
            ...
            "偏離 TL": f"{dist_pct:+.1f}%",
            "AI 市場型態": "\n".join(
                [f"L1｜{p}" for p in patterns_L1] +
                [f"L2｜{p}" for p in patterns_L2]
            ),
        }) 
